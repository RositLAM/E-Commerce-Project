          ...
          <table class="w-full">
            ...
            <tbody>
              ...
              @forelse ($cart_items as $items)
                ...
                <td><button wire:click='removeItem({{ $item['product_id'] }})' class="bg-slate-300 border-2 border-slate-400 rounded-lg px-3 py-1 hover:bg-red-500 hover:text-white hover:border-red-700"><span wire:loading.remove wire:target='removeItem({{ $item['product_id'] }})'>Remove</span wire:loading wire:target='removeItem({{ $item['product_id'] }})'>Removing...<span></span></button></td>
              </tr>
              @empty
              <tr>
                <td colspan="5" class="text-center py-4 text-4x1 font-semibold text-slate-500">No Items Availabe in Cart!</td>
              </tr>
              @endforelse
            </tbody>
          </table>
